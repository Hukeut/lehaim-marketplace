    ...
    <form action={formAction} className="flex flex-1 flex-col">
      <div className="flex-1 overflow-y-auto px-6 pt-3 pb-4">
        <div className="mb-5.5 flex justify-center">
          <div className="relative">
            ...
            <span className="absolute end-0 bottom-0 flex size-7 items-center justify-center rounded-full bg-ink text-white ring-3 ring-cream">
              <Pencil size={13} />
            </span>
          </div>
        </div>
        ...
        <div className="flex flex-col gap-3.5">
          <div className="flex gap-2.5">
            <Field label={t("firstNameLabel")} className="flex-1">
              <input
                name="first_name"
            ...
            <Field label={t("lastNameLabel")} className="flex-1">
              <input
                name="last_name"
          ...
          <Field label={t("emailLabel")}>
            ...
            />
            <p id="email-note" className="mt-1.5 text-[12.5px] text-ink/45">
              {t("emailNote")}
            </p>
          </Field>
        </div>
        ...
        {state.message && (
          <p
            role="status"
            className={`mt-4 rounded-field px-3.5 py-2.5 text-[13.5px] font-bold ${
              state.ok
      ...
      <StickyFooter className="px-6">
        <Button type="submit" disabled={!editable || pending}>
          {pending ? t("saving") : editable ? t("save") : t("loginToEdit")}
        </Button>
      </StickyFooter>
    </form>
  );
