...
export function SwapForm({
  ...
  const t = useTranslations("missions.release");
  ...
  return (
    <>
      <form action={formAction} id="swap-form" className="flex-1 overflow-y-auto px-5.5 pb-4">
        ...
        <p className="mb-4 text-[14.5px] leading-relaxed text-ink/60">
          {t("reassureText")}
        </p>
        ...
        {candidates.length ? (
                  ...
                  >
                    ...
                    >
                      ...
                      <span className="min-w-0 flex-1">
                        ...
                        <span className="block text-[12px] text-ink/65">{person.detail}</span>
                      </span>
                      <span
          ...
          <p className="rounded-field border-[1.5px] border-dashed border-line bg-white px-3.5 py-3 text-[14px] text-ink/45">
            {t("noOneElseConfirmed")}
          </p>
        )}

        {state.message && (
          <p className="mt-3 text-[13px] font-bold text-ink/60">{state.message}</p>
        )}
      </form>

      <StickyFooter className="flex flex-col gap-2.5 px-5.5">
        <Button type="submit" form="swap-form" size="md" disabled={!target || pending}>
          {chosen ? t("sendRequestTo", { name: chosen.name }) : t("chooseSomeone")}
        </Button>
